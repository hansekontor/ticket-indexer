    ...
    async unindexBlock(meta, block, view) {
		...
		if (rawIssueBrecord) {
			...
			for (let i = 1 + uInt32BEToInt(issueBrecord.start); i <= uInt32BEToInt(issueBrecord.last); i++) {
				...
				const issueTx = await this.txindex.getMeta(issueHash);
				...
				for (const addr of payoutAddresses) {
					...
					this.db.del(layout.M.del(prefix, addrHash, meta.height, i));
				}			
			}
		...
		if (rawRedeemBrecord) {
			...
			for (let i = 1; i <= numberOfRedeemTxs; i++) {
				...
				for (const addr of payoutAddresses) {
					...
					this.db.put(layout.M.encode(prefix, addrHash, issueTx.height, issueTx.index), null);
