        ...
        for (let i = 0; i < bblock.txs.length; i++) {
			...
			if (validRedeemTx) {
				...
				const issueHashIndexInt = uInt32BEToInt(issueHashIndex);
                                ...
                                //console.log("put E","key", redeemHashIndexInt, "value", redeemHash);
                                //console.log("put e", "key", redeemHash, "value", redeemHashIndex);
                                //console.log("put r", "key", issueHashIndexInt, redeemHashIndex); 
				...
				// delete issue hash index -> hash index
				// inverse table is required for unindexing
...
return;
console.log("unindexBlock()");
		// issue txs can simply be deleted
		const rawIssueBrecord = await this.db.get(layout.I.encode(meta.height));
console.log("rawIssueBrecord", rawIssueBrecord);
		if (rawIssueBrecord) {
			const issueBrecord = BlockRecord.fromRaw(rawIssueBrecord);

			// delete hash index <-> hash tables
			for (let i = uInt32BEToInt(issueBrecord.start); i <= uInt32BEToInt(issueBrecord.last); i++) {
				const issueHashIndexInt = i;
				const issueHash = await this.db.get(layout.E.encode(issueHashIndexInt));
				this.db.del(layout.d.encode(issueHash));
				this.db.del(layout.D.encode(issueHashIndexInt));
			}			
		...
		// redeem txs can be deleted after re-indexing the respective issue tx
		const rawRedeemBrecord = await this.db.get(layout.P.encode(meta.height));
console.log("rawREdeemBrecord", rawRedeemBrecord);
		if (rawRedeemBrecord) {
			const redeemBrecord = BlockRecord.fromRaw(rawRedeemBrecord);
			console.log("redeemBrecord", redeemBrecord);
			const numberOfRedeemTxs = uInt32BEToInt(redeemBrecord.last) - uInt32BEToInt(redeemBrecord.start) + 1;
			console.log("numberOfRedeemTxs", numberOfRedeemTxs);
			for (let i = 0; i <= numberOfRedeemTxs; i++) {
console.log("i", i);
				const redeemHashIndexInt = i + uInt32BEToInt(redeemBrecord.start);
console.log("get E with redeemHashIndex", redeemHashIndexInt, typeof redeemHashIndexInt);		
...
consolee.log("redeemMeta", redeemMeta);
				const redeemInputScript = redeemMeta.tx.inputs[0].script;
console.log("replace by getREdeem() script", redeemInputScript);			
	const rawIssueTx = redeemInputScript.getData(5);
				const issueTx = MTX.fromRaw(rawIssueTx);
				const issueHash = issueTx.hash();
console.log("put d", issueHash);
				const issueHashIndex = await this.db.get(layout.d.encode(issueHash));
				const issueHashIndexInt = uInt32BEToInt(issueHashIndex);

				// re-index previously deleted table D: issue hash index -> issue hash
console.log("put D", issueHashIndexInt);
				await this.db.put(layout.D.encode(issueHashIndexInt), issueHash);

				// re-index tx dummy by address
				// do later: replace the next step by something more efficient
				const validIssueTx = await validateIssueTx(issueTx);
console.log("unindex validIssueTx", validIssueTx);
				const payoutAddresses = validIssueTx;
				for (const addr of payoutAddresses) {
					const prefix = addr.getPrefix(this.network.type);
					if (prefix < 0)
					...
					const addrHash = addr.getHash();
console.log("put M", prefix, addrHash, meta.height, i);
					this.db.put(layout.M.encode(prefix, addrHash, meta.height, i), null);
				}			

				// delete in hash index <-> hash tables
				this.db.del(layout.e.encode(redeemHash));
				this.db.del(layout.E.encode(redeemHashIndex));

				// delete in issue <-> redeem table
				this.db.del(layout.r.encode(redeemHashIndex));
			}
	...
	//console.log("prevoutHash", prevoutHash);
		const issueHashIndex = await this.db.get(layout.d.encode(prevoutHash))
	//console.log("issueHashIndex", issueHashIndex);
   ...
   */
	async getLastHashIndex(height, type) {
		...
		let prevHeight = height && height > 0 ? height - 1 : 0;
		//console.log(type, "prevHeight", prevHeight);
		let prevBlockDb;
		if (type === 'issue') {
			do {
				// console.log("do-while loop with prevHeight", prevHeight);
				prevBlockDb = await this.db.get(layout.I.encode(prevHeight));
				prevHeight = prevHeight - 1;
			} while(!prevBlockDb && prevHeight >= 866600);
	...
	async getMetaByAddress(addr, type) {
		const metas = await this.getRedeemMetaByAddress(addr);
		let metasJson = [];
	...
	async getMetaByHeight(range) {
		const numberOfHeights = range.last-range.start+1;
		const heights = Array.from({length: numberOfHeights}, (_,i) => i+1)
		...
		for (const height of heights) {
			...
			const opts = {
				gte: layout.E.min(uInt32BEToInt(brecord.start)),
...
console.log("blockHeader", blockHeader);
	...
	async getIssueTx(hash) {
console.log("getIssueTx(hash)");
console.log("hash", hash);
		const issueHashIndex = await this.db.get(layout.d.encode(hash));
		console.log("issueHashIndex", issueHashIndex);
		if (!issueHashIndex) {
			console.log("TX is not an issue tx");
			return;
		}
