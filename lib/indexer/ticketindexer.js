...
class TicketIndexer extends Indexer {
   ...
   */
	...
	async getTokenUtxosByAddress(addr) {
		const slp = true;  
		const address = Address.fromString(address, 'main');

		const coins = [];

		const memCoins = this.mempool.getCoinsByAddress(address)
		...
		const blockCoins = await this.chain.getCoinsByAddress(address);
