...
class TicketIndexer extends Indexer {
    ...
    async indexBlock(meta, block, view) {
        ...
        for (let i = 0; i < bblock.txs.length; i++) {
			...
			if (validRedeemTx) {
				...
				await this.db.put(layout.r.encode(issueHash), redeemHashIndex);
    ...
    async unindexBlock(meta, block, view) {
		...
		for (let i = issueBrecord.start; i <= issueBrecord.last; i++) {
			const issueHashIndex = i;
			const issueHash = await this.db.get(layout.E.encode(issueHashIndex));
			this.db.del(layout.d.encode(issueHash));
			this.db.del(layout.D.encode(issueHashIndex));
			this.db.del(layout.I.encode(meta.height));
		}

		// redeem txs can be deleted after re-indexing the respective issue tx
		for (let i = redeemBrecord.start; i <= redeemBrecord.last; i++) {
			const redeemHashIndex = i;
			const redeemHash = await this.db.get(layout.E.encode(redeemHashIndex));
			const redeemMeta = await this.txindex.getMeta(redeemHash);
			const redeemInputScript = redeemMeta.inputs[0].script;
			console.log("fails ig this is not a bcash.Script already:", redeemInputScript);
			const rawIssueTx = redeemInputScript.getData(5);
			console.log("fails if false choice: TXMeta/TX");
			const issueMeta = new TXMeta.fromRaw();
			const issueHash = issueMeta.hash();
			console.log("fails if this does not get block:");
			const issueHashIndex = await this.db.get(layout.d.encode(issueHash));
			const issueHashIndexInt = uInt32BEToInt(issueHashIndex);
			console.log("old issueHashIndex found", issueHashIndexInt);
			...
			// delete all redeem tx related data
		...
		// delete block header
		this.db.del(layout.B.encode(bblock.hash()));
