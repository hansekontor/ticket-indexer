...
class TicketIndexer extends Indexer {
   ...
   */
	...
	async getMetaJsonsByAddress(addr) {
		const limit = 10;
		let metas;
		metas = await this.getIssueMetaByAddress(addr);
		
		const redeemTxsLimit = metas.length > limit ? false : limit - metas.length;
		if (redeemTxsLimit) {
			const redeemMetas = await this.getRedeemMetaByAddress(addr, redeemTxLimit);
			metas.concat(redeemMetas);
		}
	...
	async getMetaByAddress(addr, type) {
		...
		const confirmed = async () => {
			...
			if (type === "issue") 
				...
				hashes = await this.getRedeemHashesByAddress(addr);
	...
	async getMetaByHeight(range) {
		const count = range.last - range.start + 1;
