...
console.log("get I by ", meta.height);
...
//console.log("redeemMeta", redeemMeta);
...
console.log("put M", prefix, addrHash, meta.height, i);
   ...
   */
	async getLastHashIndex(height, type) {
		...
		let cursor = prevHeight;
		//console.log(type, "prevHeight", prevHeight);
		let prevBlockDb;
		if (type === 'issue') {
			do {
				console.log("do-while loop with cursor", cursor);
				prevBlockDb = await this.db.get(layout.I.encode(cursor));
				cursor = cursor - 1;
			} while(!prevBlockDb && cursor >= 866600);

		} else {
			do {
				console.log("do-while loop with cursor", cursor);
				prevBlockDb = await this.db.get(layout.P.encode(cursor));
				cursor = cursor - 1;
			} while(!prevBlockDb && cursor >= 866600);
		}
