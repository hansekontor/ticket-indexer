...
console.log("getMetaByHeight() with range", range);
...
console.log("hashIndexes", hashIndexes);			// get all hashes for height
			const hashes = [];
			for (const hashIndex of hashIndexes) {
				const hash = await this.db.get(layout.E.encode(hashIndex));
				hashes.push(hash);
			}
	...
	async getBlockHeader(height) {
		const hash = await this.txindex.get(layout.h.encode(height));
	...
	async getMetaJson(meta) {
        ...
        const metaJson = meta.getJSON(this.network, view, this.chain.height);
        if (slp) {
