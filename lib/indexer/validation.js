...
//console.log("validating", tx.hash());
	// validate p2sh address
	let auth;
	const vout1Output = tx.outputs[1];
	if (!vout1Output)
		return false;
//console.log("vout1Output", vout1Output);
	const vout1Address = vout1Output.getAddress();
//console.log("vout1Address", vout1Address);
	const isOpReturn = vout1Address === null;
//console.log("isOpReturn", isOpReturn);
	if (isOpReturn)
		return false;
	...
	if (!isScriptHash) 
		return false;
//console.log("isScriptHash", isScriptHash);
...
//console.log("auth", auth);
	...
	const outpoint = input.prevout;
	const outpointBuf = outpoint.toRaw();
	const vout0Output = tx.outputs[0];	
	const authCodeBuf = vout0Output.script.get(1).data;
...
function readTicketAuthCode(authCodeBuf) {
    ...
    const outputsReader = bio.read(txSerializedOutputs);
    const txOutputs = [];
