...
const { Address, Script, script, bcrypto, KeyRing } = bcash;
const { SLP } = script;
...
function validateIssueTx(tx) {
	...
	const givenP2SHAddress = tx.outputs[1].address;
...
module.exports = validateIssueTx;
...
function getInputPubkey(scriptString) {
	const sigScriptBuffer = Buffer.from(scriptString, 'hex');
	const br = bio.read(sigScriptBuffer);
